...
BOOL xf_rail_adjust_position(xfContext* xfc, xfAppWindow* appWindow)
{
	...
	const INT32 rdpX = appWindow->x - freerdp_settings_get_int32(settings, FreeRDP_MonitorLocalShiftX);
	const INT32 rdpY = appWindow->y - freerdp_settings_get_int32(settings, FreeRDP_MonitorLocalShiftY);
	    ...
	    appWindow->height != (INT64)appWindow->windowHeight)
	{
		...
		windowMove.bottom =
		    WINPR_ASSERTING_INT_CAST(INT16, rdpY + appWindow->height + bottom);
...
BOOL xf_rail_end_local_move(xfContext* xfc, xfAppWindow* appWindow)
{
	...
	appWindow->windowOffsetX = appWindow->x - freerdp_settings_get_int32(settings, FreeRDP_MonitorLocalShiftX);
	appWindow->windowOffsetY = appWindow->y - freerdp_settings_get_int32(settings, FreeRDP_MonitorLocalShiftY);
...
BOOL xf_rail_paint_surface(xfContext* xfc, UINT64 windowId, const RECTANGLE_16* rect)
{
	...
	const RECTANGLE_16 windowRect = {
		...
		.right = WINPR_ASSERTING_INT_CAST(UINT16, MAX(appWindow->windowOffsetX + appWindow->width, 0)),
		.bottom = WINPR_ASSERTING_INT_CAST(UINT16, MAX(appWindow->windowOffsetY + appWindow->height, 0))
	};
                                  ...
                                  const WINDOW_STATE_ORDER* windowState)
{
	...
	if (position_or_size_updated)
	{
		...
		if (appWindow->rail_state != WINDOW_SHOW_MINIMIZED)
		{
			...
			if (appWindow->x == expectedX &&
			    appWindow->y == expectedY &&
			    appWindow->width == (INT64)appWindow->windowWidth &&
			    appWindow->height == (INT64)appWindow->windowHeight)
			{
